...
function writeWord() {
  const w = new Word();
  let str = findWord();
  written.push(w.setXYAndString(mouseX-20, mouseY, str, wordSize));
  redraw();
}

class Word {
 // x and y values for position at which the word is drawn
  constructor(x, y, str) { this.setXYAndString(x, y, wordSize); }
 
  setXYAndString(x, y, str, wordSize) {
    this.x = x, this.y = y, this.str = str, this.size = wordSize;
    return this;
  }
